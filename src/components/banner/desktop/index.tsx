    ...
    <div className={`container-fluid ${styles.banner}`}>
      <div className={`row h-100 justify-content-center align-items-center`}>
        <div className={`col-10 text-center mx-auto`}>
          <h2 className={styles.title}>{t('Supercharge your career')}</h2>
          ...
          <div className={`d-flex justify-content-center ${styles.flexGap}`}>
            <button type="button" onClick={() => Router.push(findAMentorLink)} className={`btn bg-white btn-outline-dark ${styles.button}`}>{t('Find a mentor')}</button>
            <button type="button" onClick={() => Router.push(findAMentorLink)} className={`btn bg-white btn-outline-dark ${styles.button}`}>{t('Become a mentor')}</button>
          </div>
        </div>
      </div>
    </div>
